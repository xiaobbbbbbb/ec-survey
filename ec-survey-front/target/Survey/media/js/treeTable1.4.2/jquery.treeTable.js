...
(function ($) {
	...
	if (document.location.href.indexOf('http://') != 0)	{
		var path = '../../treeTable1.4.2/';
	} else {
		var path = SITE_URL + '/../treeTable1.4.2/';
	}*/
    ...
    $.fn.treeTable = function (opts) {
       ...
       /* if ($('head').find('#tree_table_' + opts.theme).length == 0) {
        ...
        function formatNode(tr) {
            ...
            //-------------下面一大段都是获取$preSpan---------
            if (cMap[id] == 0) {
                //如果是顶级节点，则没有prev_s_pan
                var $preSpan = $('<div class="prev_s_pan"></div>');
            } else {
                //先判断是否有上一个兄弟节点
                if (!$cur.attr('isFirstOne')) {
                    var $preSpan = $('#' + $cur.attr('prevId'), $treeTable).children("td").eq(opts.column).find('.prev_s_pan').clone();
                } else {
                    var $parent = $('#' + cMap[id], $treeTable);
                    //没有上一个兄弟节点，则使用父节点的prev_s_pan
                    var $preSpan = $parent.children("td").eq(opts.column).find('.prev_s_pan').clone();

                    //如果父亲后面没有兄弟，则直接加空白，若有则加竖线
                    if ($parent.attr('isLastOne')) {
                        $preSpan.append('<div class="' + css['N'] + ' ' + css['B'] + '"></div>');
                    } else {
                        $preSpan.append('<div class="' + css['N'] + ' ' + css['V'] + '"></div>');
                    }
                }
            }
            //------------------------------------------------
            ...
            $td.prepend('<div arrow="true" class="' + className + '"></div>').prepend($preSpan);
        };
